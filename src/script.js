...
window.addEventListener("resize", () => {
  ...
  sizes.pixelRatio = Math.min(window.devicePixelRatio, 2); // For an external display, the pixel ratio can be greater than 2
...
const createFirework = (count, position, size, texture, radius, color) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const spherical = new THREE.Spherical(
      radius * (0.75 + Math.random() * 0.25), // Random radius between 0.75 and 1 for the spherical coordinates of the position of the particles of the firework
      Math.random() * Math.PI,
    ...
    // Random position between -1 and 1 for each axis (x, y, z) of the position attribute of the geometry of the particles of the firework (positionsArray)
...
const createRandomFirework = () => {
  ...
  // const texture = textures[8]; // Gabbe texture
